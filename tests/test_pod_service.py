...
class TestJoinMission:
    ...
    def test_mission_not_found(self, mock_get_mission):
        from OrbitServer.services.pod_service import join_mission
        mock_get_mission.return_value = None

        pod, err = join_mission(999, 1)
        assert pod is None
        assert "not found" in err.lower()

    @patch('OrbitServer.services.pod_service.get_mission')
    def test_mission_not_open(self, mock_get_mission):
        from OrbitServer.services.pod_service import join_mission
        mock_get_mission.return_value = {'id': 1, 'status': 'completed', 'max_pod_size': 4}

        pod, err = join_mission(1, 1)
        assert pod is None
        assert err is not None

    @patch('OrbitServer.services.pod_service.get_user_pod_for_mission')
    @patch('OrbitServer.services.pod_service.get_mission')
    def test_returns_existing_pod(self, mock_get_mission, mock_existing):
        from OrbitServer.services.pod_service import join_mission
        mock_get_mission.return_value = {'id': 1, 'status': 'open', 'max_pod_size': 4}
        mock_existing.return_value = {'id': 'pod-abc', 'mission_id': 1}

        pod, err = join_mission(1, 5)
        assert err is None
        assert pod['id'] == 'pod-abc'

    @patch('OrbitServer.services.pod_service.record_action')
    ...
    @patch('OrbitServer.services.pod_service.get_user')
    @patch('OrbitServer.services.pod_service.get_user_pod_for_mission')
    @patch('OrbitServer.services.pod_service.get_mission')
    def test_joins_existing_open_pod(self, mock_mission, mock_existing, mock_user,
                                     mock_best, mock_trans, mock_record):
        from OrbitServer.services.pod_service import join_mission
        mock_mission.return_value = {'id': 1, 'status': 'open', 'max_pod_size': 4, 'tags': []}
        mock_existing.return_value = None
        mock_user.return_value = {'interests': []}
    ...
    @patch('OrbitServer.services.pod_service.record_action')
    @patch('OrbitServer.services.pod_service.create_pod')
    @patch('OrbitServer.services.pod_service._find_best_pod_for_user')
    @patch('OrbitServer.services.pod_service.get_user')
    @patch('OrbitServer.services.pod_service.get_user_pod_for_mission')
    @patch('OrbitServer.services.pod_service.get_mission')
    def test_creates_new_pod_when_none_open(self, mock_mission, mock_existing, mock_user,
    ...
    @patch('OrbitServer.services.pod_service.get_user_pod_for_mission')
    @patch('OrbitServer.services.pod_service.get_mission')
    def test_pod_marked_full_when_at_cap(self, mock_mission, mock_existing, mock_user,
                                         mock_best, mock_trans, mock_record):
        from OrbitServer.services.pod_service import join_mission
        mock_mission.return_value = {'id': 1, 'status': 'open', 'max_pod_size': 4, 'tags': []}
        mock_existing.return_value = None
    ...
    @patch('OrbitServer.services.pod_service.get_user_pod_for_mission')
    @patch('OrbitServer.services.pod_service.get_mission')
    def test_records_joined_action(self, mock_mission, mock_existing, mock_user,
        ...
        mock_user.return_value = {'interests': []}
        mock_best.return_value = None
        with patch('OrbitServer.services.pod_service.create_pod') as mock_create:
            mock_create.return_value = {'id': 'pod-new', 'member_ids': [5]}
            join_mission(1, 5)
    ...
    @patch('OrbitServer.services.pod_service.get_user_pod_for_mission')
    @patch('OrbitServer.services.pod_service.get_mission')
    def test_creates_new_pod_when_transaction_finds_full(self, mock_mission, mock_existing,
                                                         mock_user, mock_best,
...
class TestLeaveMission:
    ...
    def test_not_in_pod(self, mock_existing):
        from OrbitServer.services.pod_service import leave_mission
        mock_existing.return_value = None

        result, err = leave_mission(1, 5)
...
class TestFindBestPodForUser:
    @patch('OrbitServer.services.pod_service.list_pods')
    ...
    @patch('OrbitServer.services.pod_service.get_user')
    @patch('OrbitServer.services.pod_service.list_pods')
    def test_selects_most_compatible_pod(self, mock_list, mock_user):
        from OrbitServer.services.pod_service import _find_best_pod_for_user
        ...
        mock_user.side_effect = user_side_effect
    ...
    @patch('OrbitServer.services.pod_service.get_user')
    @patch('OrbitServer.services.pod_service.list_pods')
    def test_single_open_pod_returned(self, mock_list, mock_user):
        from OrbitServer.services.pod_service import _find_best_pod_for_user
        pod = {'id': 'pod-1', 'member_ids': [1], 'status': 'open'}
        mock_list.return_value = [pod]
        mock_user.return_value = {'interests': ['art']}
